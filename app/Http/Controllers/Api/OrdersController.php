...
class OrdersController extends Controller
{
    ...
    {
        // Inizializza $orderItemsData come array vuoto
        $orderItemsData = [];

        try {
            // Verifica se 'orderItems' è presente nella richiesta
            if ($request->has('orderItems')) {
                ...
                $requestForValidation = new Request([
                    'orderItems' => $orderItemsData,
                ]);

                // Valida i dati dell'ordine
                $this->validate($requestForValidation, [
